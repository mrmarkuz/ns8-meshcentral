...
# Reuse existing nodebuilder-meshcentral container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-meshcentral; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-meshcentral -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/mongo:7.0.20 docker.io/typhonragewind/meshcentral:preloadlibs-mongodb-1.1.44" \
    "${container}"
# Commit the image
